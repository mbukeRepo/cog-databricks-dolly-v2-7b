...
DEFAULT_MODEL = "databricks/dolly-v2-7b"
...
PATH_TO_TENSORIZER_WEIGHTS = REMOTE_PATH_TO_TENSORIZER_WEIGHTS if REMOTE_PATH_TO_TENSORIZER_WEIGHTS else "./tensorized_models/databricks-dolly-v2-7b.tensors"
...
class Predictor(BasePredictor):
    ...
    def load_huggingface_model(self, weights=None):
        st = time.time()
        logger.info(f'loading weights from {weights} w/o tensorizer')
        model = YieldingCausalLM.from_pretrained(
            weights, cache_dir=CACHE_DIR
        ).to("cuda:0")
        logger.info(f'weights loaded in {time.time() - st}')
        return model
